...
public final class TextFieldDefaultsKt {
    ...
    public static final State<BorderStroke> m1873animateBorderStrokeAsStateNuRrP5Q(boolean enabled, boolean isError, InteractionSource interactionSource, TextFieldColors colors, float focusedBorderThickness, float unfocusedBorderThickness, Composer $composer, int $changed) {
        ...
        if (enabled) {
            ...
            stateRememberUpdatedState = AnimateAsStateKt.m80animateDpAsStateAjpBEmI(targetThickness, AnimationSpecKt.tween$default(150, 0, null, 6, null), null, null, $composer, 48, 12);
            $composer.endReplaceableGroup();
        } else {
            $composer.startReplaceableGroup(-1927737286);
            ComposerKt.sourceInformation($composer, "2118@112993L46");
            stateRememberUpdatedState = SnapshotStateKt.rememberUpdatedState(Dp.m5272boximpl(unfocusedBorderThickness), $composer, ($changed >> 15) & 14);
            $composer.endReplaceableGroup();
        }
        State<BorderStroke> stateRememberUpdatedState2 = SnapshotStateKt.rememberUpdatedState(new BorderStroke(stateRememberUpdatedState.getValue().m5288unboximpl(), new SolidColor(stateIndicatorColor$material3_release.getValue().m2981unboximpl(), null), null), $composer, 0);
        if (ComposerKt.isTraceInProgress()) {
